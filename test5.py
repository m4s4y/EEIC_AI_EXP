# 単純なビームサーチだと "sil" 全埋めが生じた。 "sil" は各シーンについて最大一回しか現れないようになっているので制限をかけてみた。
...
def beam_search_decoder(y, beam_width=5, blank_token=0, sil_token=43):  # arr2txt で letters[idx - start] に変換するので。 42+1
    """
    ビームサーチアルゴリズムによるデコード
    :param y: モデルの出力 (max_seq_len, batch_size, num_classes)
    ...
    for t in range(seq_len):  # 時間ステップごとにループ
        ...
        for i in range(batch_size):  # バッチごとに処理
            ...
            if any(sil_used for _, _, sil_used in beams[i]):
                ...
                filtered_candidates = [c for c in candidates if c[1][-1] != sil_token]
