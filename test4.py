...
ビームサーチの改善版。
- ブランクトークンのバイアスを強化
- CTC-loss での length_penalty の調整
- ビームサーチにおいて beam_width（探索幅）の縮小
'''
import torch
...
# ブランクトークンのバイアスを強化
def adjust_output_for_blank_token(output, blank_bias=-50):
    """
    出力にバイアスを加え、ブランクトークン（index=0）のスコアを強く下げる。
    :param output: モデルの出力（logits）
    :param blank_bias: ブランクトークンに対して追加する強いバイアス
    :return: バイアスを加えた出力
    """
    output[:, :, 0] += blank_bias  # 0はブランクトークンのインデックス
    return output
