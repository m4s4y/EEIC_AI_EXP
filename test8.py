...
# モデルの最終層のバイアスを加える
def adjust_output_for_blank_token(output, blank_bias=-5):
...
def train(model, net):   # こいつだと、なんかエラー出る？音素列が文字列なんたらかんたら
    ...
    for epoch in range(opt.max_epoch):
        for (i_iter, input) in enumerate(loader):
            ...
            # loss_with_penalty = ctc_loss - torch.mean(length_rewards)
            # loss = loss_with_penalty + torch.mean(consonants_pena)
            loss = ctc_loss + torch.mean(consonants_pena)
            ...
            if(tot_iter % opt.test_step == 0):                
                ...
                savename = '{}_test8_only_train_without_blank_len_{}.pt'.format(opt.save_prefix, count)
                log("save the model")
                count += 1
